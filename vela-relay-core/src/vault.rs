...
///
/// Exposed to the shell (the only other workspace crate) for its signing
/// executors; neither crate is published, so private-key material still never
/// reaches a public API. `k256::SecretKey` redacts `Debug` output and zeroizes
/// its bytes on drop.
pub fn derive_treasury_secret_key(operator_secret: &str) -> Result<SecretKey, VaultError> {
...
///
/// Exposed to the shell for its signing executors; see
/// [`derive_treasury_secret_key`] for the custody rationale.
pub fn derive_pool_relayer_secret_key(
    operator_secret: &str,
    index: usize,
