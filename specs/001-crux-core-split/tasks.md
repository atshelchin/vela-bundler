...
- [x] T034 [US4] Composite decomposition COMPLETE (three batches). **Batch 1**: broadcast sequencing (`CheckBroadcastSeen`/`BroadcastRaw`/cache ops/probes/`ClearStaleIntent`) judged by `crate::broadcast`; nonce triage (`FetchAccountNonces` + defer/reject causes); the Gnosis xDAI peg (`settlement::pegged_native_usd_price`); the `2×base+tip` and legacy-tip rules (`gas_math`). **Batch 2a**: the whole Tempo `0x76` tail as an in-core continuation (fee-token gate, pathUSD settlement gate, `tempo_handle_ops_gas_limit`/`tempo_outer_max_fee` rules, funding-sufficiency floor, reuse of the decomposed broadcast). **Batch 2b**: treasury funding as in-core sequences for native and Tempo (lease acquire/heartbeat/release with a batch-end backstop, prepared-intent resume with the receipt probe, plan/affordability over the core rules, treasury sign requests, funding broadcast without a stale-nonce path); `PreparedFundingIntent` joined the core vocabulary. **Batch 2c**: `audit_bundle_replay` (the prepared-bundle member classification with its integrity refusals) became a pure core rule with a matrix test. *Deliberate residue: `ResumeBundleIntent` remains a composite operation and the `reconcile_prepared_bundles` timer loop stays in the shell — every RULE they apply (audit classification, receipt success, broadcast judgement) now lives in the core; what remains in the shell is IO orchestration only.*
