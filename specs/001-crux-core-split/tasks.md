...
- [ ] T034 [US4] Decompose the remaining composite operations into core-driven sequences: `ResumeBundleIntent` (+ `reconcile_prepared_bundles`), `ResolveNonceMismatches`, `EnsureRelayerFunded` (treasury lease/probe/sign/broadcast), `BroadcastBundle` (sequencing the `crate::broadcast` judgement), `ExecuteTempoBundle`, plus the Gnosis-peg rule inside the shell's `FetchMarketPrice` and the fee math inside `transaction_context`
