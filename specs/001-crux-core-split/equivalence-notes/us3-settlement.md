...
| Log lines: "in-band reimbursement cannot fund an includable outer fee" (fields `chain_id, quoted_fee, affordable, floor, base_fee`) and "repriced the outer transaction to the signed in-band budget" (fields `chain_id, quoted_fee, repriced_fee, base_fee, tip`) | Emitted by the shell from the `FloorUnfundable` / `Reprice` arms with identical messages and field names |
