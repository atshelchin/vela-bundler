...
- [ ] T007 [P] [US1] `vela-relay-cf/src/config.rs`: env/secrets → validated policy values (same defaults/bounds as the docker parser; shared numeric defaults promoted to core consts where absent — additive); dynamic-chain posture per R10 (EXECUTION_CHAINS optional), lane width 1..=100 per R11
- [ ] T008 [P] [US1] `vela-relay-cf/src/record_do.rs`: RecordDO — serde fetch protocol (`create_queued` if-absent, `get`, `mark_admitted`, `patch` via `lifecycle::decide_patch`, `mark_bundle_member_submitted` via `decide_bundle_submission`); TTL alarm cleanup (earliest-of packing per data-model §1); storage layout documented in the file header
- [ ] T009 [US1] `vela-relay-cf/src/http.rs`: fetch handler routing (`/`, `/health`, `/healthz`, `/readyz` as binding checks per deployment-parity.md, `/version`, `POST /{chain_id}`) → wire dispatch; reads (supportedEntryPoints, status, byHash, receipt) via RecordDO + wire renderers
- [ ] T010 [US1] AdmissionApp driver in the fetch path + arms: `LoadSettlementAssets` (chain-directory fetch + KV cache), `FetchTokenDecimals` (RPC fetch via arms/rpc.rs), `CreateQueued`/`LoadExisting`/`MarkAdmitted` → RecordDO, `Enqueue` → queue producer (send failure → `QueueUnavailable`, crash-window semantics preserved); responses rendered by wire
...
- [ ] T012 [US1] Gate 2 run: replay battery against `wrangler dev` AND the docker build, `diff -r` on bodies; scripted crash-window check (forced queue-send failure → response + RecordDO state match docker's); fix until byte-clean; update `contracts/platform-bindings.md` cells that changed during implementation
