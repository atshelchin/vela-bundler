...
- [ ] T022 [US4] Load harness (k6 or fetch-based) in `specs/002-cf-worker-shell/load/`: sustained ≥1,000 submits/s + ≥10,000 reads/s from three regions, 30 min, p95 targets; then per-chain isolation experiment (SC-007); tune queue batch size/consumer concurrency from results; record numbers in quickstart. *Harness BUILT and smoke-verified (specs/002-cf-worker-shell/load/): load.js encodes SC-004 as k6 scenarios+thresholds (constant-arrival submits/reads, p95 500/200 ms, failure rate <0.1%, per-region nonce salting, accepted-only counting via the JS port of the payment-calldata builder — smoke vs wrangler dev: 603/603 checks, 101/101 submissions accepted, zero refusals) and isolation.js encodes SC-007 (baseline vs saturated runs, chain-tagged p95 comparison — smoke 257/257). DEPLOYED AND PARTIALLY RUN (2026-08-29): the real deployment is live at https://vela-relay-cf.atshelchin.workers.dev (queues + auto-provisioned KV + 3 DO migrations; fresh OPERATOR_SECRET exclusive to CF per FR-010 — vault 0x3e59292e…fe3c, saved in gitignored .env.cloudflare; executor disabled pending treasury funding). Production Gate 2 replay: 42/42 byte-identical vs a docker reference paying the same vault. SC-007 isolation PASSED on the real edge (victim p95 661 ms under 300/s cross-chain saturation vs 701 ms baseline — <0% degradation ≪ 10%; 50,396/50,396 checks). SC-004 sample at 1/10 rate, single vantage: 50 submits/s + 500 reads/s held 4 min, 143,939/143,939 checks, 0/131,938 failures, 12,001 ops accepted into real DOs+Queues; the full three-region 30-min p95 claim remains open (needs regional load generators — result table in load/README.md)*
